...
export default function Home(props: { menuData: string[],haveLunch:string,id:string,role:string ,name:string}) {
  ...
  return (
    ...
    <div className={`bg-[#55647471] ${showPopUp?'opacity-60 transition-opacity pointer-events-none':''}`}>
      <nav className="pr-4 md:h-[70px] h-[50px] flex justify-between items-center relative bg-black md:bg-transparent  ">
      ...
      <div className="w-full homeHeight  flex justify-center ">
        ...
        <div className="md:w-[50%] w-full h-full pt-5 text-center">
          ...
          <button
            className="mt-5 p-2  font-semibold bg-black text-white  rounded-md text-md active:bg-zinc-500 z-1"
            onClick={submitClick}
          >
        Submit
          </button>
      ...
      <div className=" border-box h-[28px] text-xs items-center justify-center text-center fixed bottom-0 right-0 left-0 bg-black text-white">
        &copy; 2023 SDETTECH. All rights reserved.
      </div>
      </div>
      {showPopUp &&<div><SubmitPopUp handleClose={()=>{setShowPopUp(!showPopUp)}}/></div>}
      {showAlert&& <div><PopUp message="Please select one of options" changeView={()=>setShowAlert(!showAlert)}/></div>}
</div>
  )}